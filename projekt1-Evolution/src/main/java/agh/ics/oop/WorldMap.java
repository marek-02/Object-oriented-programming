...
public class WorldMap {
    ...
    private int fullEnergy; // czy tym się powinna zajmować mapa?
    private int birthEnergy;
    private int minMutation; // czy tym się powinna zajmować mapa?
    ...
    public WorldMap(WorldMap map) {
        ...
        if (typeOfPlants == TypeOfPlants.EQATOR) {// if nie jest najlepszym rozwiązaniem
            int a = 2 * height / 5;
            int b = 3 * height / 5;
    ...
    public WorldMap(int width, int height, int energyFromPlant, int startPlants, int dayPlants, int startAnimals, int startEnergy, int fullEnergy,
        ...
        if (typeOfPlants == TypeOfPlants.EQATOR) { // DRY
            int a = 2 * height / 5;
            int b = 3 * height / 5;
            this.numberOfAttractiveFields = (b - a) * width;
            this.freeAttractiveFields = this.numberOfAttractiveFields;
    ...
    private void moveAnimal(Animal a, int index, TypeOfMovement type) {
        ...
        if (newX >= width || newX < 0 || newY >= height || newY < 0) {
            if (typeOfPlanet == TypeOfPlanet.PORTAL) { // if nie jest najlepszym rozwiązaniem
                Random generator = new Random();
    ...
    private void reproduceAnimal(int x, int y, Animal a1, Animal a2) { // czy to powinno być w mapie?
        Random generator = new Random(); // nowy obiekt co wywołanie
        int e1 = a1.getEnergy();
        int e2 = a2.getEnergy();
